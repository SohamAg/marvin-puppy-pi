...
model_file = "/Users/jarod/Documents/4th_Year/VIP/Project_Robot/AudioClassification/model/model_edgetpu.tflite"
...
class AudioProcessor:
  def __init__(self):
    ...
    if TPU:
      # Set up TPU
      self.tpu_inference = edgetpu.make_interpreter(model_file, device=":0")
      self.tpu_inference.allocate_tensors()
    else:
      # model used for inference
      self.model = tf.keras.models.load_model('../model/audio_inference.keras') 
...
#----------------------------- Processing thread -----------------------------#
  ...
  def process_audio(self, audio_data):
    front_padding = (WINDOW_SAMPLES - len(audio_data)) // 2
    back_padding = (WINDOW_SAMPLES - len(audio_data)) - front_padding
    padding = tf.constant([[front_padding,back_padding]])
    padded_waveform = tf.pad(audio_data, padding)
    padded_waveform = tf.expand_dims(padded_waveform, 0)

    mel_audio, label = record.mel_spectrogram(padded_waveform, 0)
    return mel_audio
